                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <ul id="myTab" class="nav nav-tabs" role="tablist">
                            <?php if(is_array($setting_category) || $setting_category instanceof \think\Collection || $setting_category instanceof \think\Paginator): $i = 0; $__LIST__ = $setting_category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?>
                            <li style="padding-right: 10px;">
                                <a href="/BJPrExO9/setting/<?php echo htmlentities($c['category_name']); ?>">
                                    <button <?php if($c['category_name'] == $cgory): ?>class="tab_btn btn btn-default"<?php else: ?>class="tab_btn btn btn-warning"<?php endif; ?>><?php echo htmlentities($c['category_name']); ?></button>
                                </a>
                            </li>
                        ...
                        <div class="card-body" style="padding: 15px 200px 10px;">
                            <?php $i=1;if(is_array($setting) || $setting instanceof \think\Collection || $setting instanceof \think\Paginator): $i = 0; $__LIST__ = $setting;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;$i++;if($s['input_type'] == 'text' || $s['input_type'] == 'number' || $s['input_type'] == 'password'): ?>
                            <div class="form-group">
                                <label for="<?php echo htmlentities($s['sign']); ?>"><?php echo htmlentities($s['title']); ?></label>
                                <input type="<?php echo htmlentities($s['input_type']); ?>" class="form-control" id="<?php echo htmlentities($s['sign']); ?>" name="<?php echo htmlentities($s['sign']); ?>" readonly value="<?php echo htmlentities($s['value']); ?>">
                                <small id="emailHelp" class="form-text text-muted text-danger"><?php echo htmlentities($s['remark']); ?></small>
                            </div>
                            <?php elseif($s['input_type'] == 'onoff'): ?>
                            <div class="form-group">
                                <label for="<?php echo htmlentities($s['sign']); ?>"><?php echo htmlentities($s['title']); ?></label><br/>
                                <input class="value" type="checkbox" id="<?php echo htmlentities($s['sign']); ?>" name="<?php echo htmlentities($s['sign']); ?>" readonly data-toggle="toggle" data-onstyle="danger" data-style="btn-round" <?php if($s['value'] == 'on'): ?>checked<?php endif; ?>>
                            </div>
                            <?php elseif($s['input_type'] == 'select'): ?>
                            <!-- <div class="form-group" id="div<?php echo htmlentities($s['sign']); ?>">
                                <label for="<?php echo htmlentities($s['sign']); ?>"><?php echo htmlentities($s['title']); ?></label>
                                <select class="form-control form-control-sm" id="<?php echo htmlentities($s['sign']); ?>" name="<?php echo htmlentities($s['sign']); ?>" onclick='add_select("<?php echo htmlentities($s['sign']); ?>", <?php echo $s['remark']; ?>)'>
                                    <option value=""><?php if($s['value'] == ''): ?>请选择<?php else: ?>当前选择:<?php echo htmlentities($s['value']); ?><?php endif; ?></option>
                                </select>
                            </div> -->
                            <?php endif; ?>
                            <?php endforeach; endif; else: echo "" ;endif; ?>
